...
public class AddOrEdit {
	...
	private void initialize() {
		...
		ArrayList<String> dataFromeDB = new ArrayList();
		dataFromeDB.add("1");
		dataFromeDB.add("2");
		
		String[] elements = new String[dataFromeDB.size()+1];
		elements[0] = "select id";
		for(int i=0; i<dataFromeDB.size(); i++){
			elements[i+1] = dataFromeDB.get(i);
		}
		if(status.equals("Edit")){
			...
			comboBox.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					posField.setText(id);
					depField.setText(id);
					permissionField.setText(id);
					salaryField.setText(id);
				}
		...
		btnSubmit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(status.equals("Edit")){
					//PersonnelManagement.editPersonnel(comboBox.getSelectedItem().toString(),
					//nameField.getText(), posField.getText(), depField.getText(), 
					//permissionField.getText(), salaryField.getText());
				}
				else if(status.equals("Add")){
					//PersonnelManagement.addPersonnel(idField.getText(),
					//nameField.getText(), posField.getText(), depField.getText(), 
					//permissionField.getText(), salaryField.getText());
				}
			}
